...
export function BrideProfileModal({ bride, onClose }: BrideProfileModalProps) {
  ...
  const memberSince = new Date(bride.createdAt).toLocaleDateString("en-AU", {
    day: "numeric",
    month: "short",
    year: "numeric",
  });
  ...
  // Parse style preferences JSON
  let stylePrefs: Record<string, string[]> = {};
  try {
    stylePrefs = profile?.stylePreferences
      ? JSON.parse(profile.stylePreferences)
      : {};
  } catch {}
  const hasStylePrefs = Object.values(stylePrefs).some((v) => v.length > 0);

  const notes = profile?.notes ?? null;
  const notesTooLong = notes && notes.length > NOTES_LIMIT;
  const displayedNotes =
    notesTooLong && !notesExpanded ? notes.slice(0, NOTES_LIMIT) + "…" : notes;
      ...
      >
        ...
        >
          ...
          >
            ...
            {weddingDate && daysUntil !== null && (
              ...
              >
                ...
                <div>
                  <div
                    style={{ fontSize: 12, color: "#D4A373", fontWeight: 600 }}
                  >
                    {daysUntil > 0
          ...
          >
              ...
              <div style={{ marginBottom: 8 }}>
                ...
                >
                  ...
                  <span style={{ fontSize: 11, color: "#AAA" }}>
                    {stageIndex + 1} of {BRIDE_STAGE_ORDER.length}
                  </span>
                </div>
                <div
                  style={{ height: 6, background: "#F0EAE2", borderRadius: 6 }}
                >
                  <div
                    style={{
                      height: 6,
            ...
            <Section title="Contact & Wedding Details">
              <div
                style={{ display: "flex", flexDirection: "column", gap: 10 }}
              >
                <InfoRow
                  icon={<Mail size={14} color="#A67C52" />}
                  label="Email"
                  value={bride.email}
                />
                <InfoRow
                  icon={<Phone size={14} color="#A67C52" />}
                  label="Phone"
                  value={profile?.phone ?? null}
                />
                <InfoRow
                  icon={<Calendar size={14} color="#A67C52" />}
                  label="Wedding Date"
                  value={weddingDate}
                />
                {profile?.partnerName && (
                  <InfoRow
                    icon={<Heart size={14} color="#A67C52" />}
                    label="Partner"
                ...
                {profile?.venueName && (
                  <InfoRow
                    icon={<MapPin size={14} color="#A67C52" />}
                    label="Venue"
                ...
                {profile?.guestCount && (
                  <InfoRow
                    icon={<Users size={14} color="#A67C52" />}
                    label="Guest Count"
                ...
                {profile?.address && (
                  <InfoRow
                    icon={<Home size={14} color="#A67C52" />}
                    label="Address"
            ...
            {hasStylePrefs && (
              <Section title="Style Preferences">
                <div
                  style={{ display: "flex", flexDirection: "column", gap: 10 }}
                >
                  {stylePrefs.styles?.length > 0 && (
                    <PrefRow label="Aesthetic" items={stylePrefs.styles} />
                  )}
                  {stylePrefs.silhouettes?.length > 0 && (
                    <PrefRow
                      label="Silhouette"
                      items={stylePrefs.silhouettes}
                    />
                  )}
                  {stylePrefs.fabrics?.length > 0 && (
                    <PrefRow label="Fabrics" items={stylePrefs.fabrics} />
                  )}
                  {stylePrefs.necklines?.length > 0 && (
                    <PrefRow label="Neckline" items={stylePrefs.necklines} />
                  )}
                </div>
              </Section>
            )}
            ...
            {profile?.dietaryNotes && (
              <Section title="Dietary / Accessibility Notes">
                <div
            ...
            {/* Notes — with expand/collapse for long text */}
            {notes && (
              <Section title="Notes">
                <div
                  style={{ display: "flex", alignItems: "flex-start", gap: 10 }}
            ...
            {!hasStylePrefs && !notes && !profile?.dietaryNotes && (
              <div
                style={{
                  textAlign: "center",
                  padding: "12px 0",
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div>
        ...
        <div
          style={{ fontSize: 13, color: value ? "#333" : "#CCC", marginTop: 1 }}
        >
          {value ?? "Not provided"}
        </div>
      </div>
