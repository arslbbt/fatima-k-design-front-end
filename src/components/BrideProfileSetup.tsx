...
// stylePreferences is stored as a JSON-like comma string; we split/join for the pill UI
function parsePrefs(raw: string | null | undefined): Record<string, string[]> {
  try {
...
function serializePrefs(prefs: Record<string, string[]>): string {
...
export function BrideProfileSetup() {
  ...
  useEffect(() => {
    if (!me) return;
    const p = me.brideProfile;
    if (p?.weddingDate) setWeddingDate(p.weddingDate.split("T")[0]);
    if (p?.partnerName) setPartnerName(p.partnerName);
    if (p?.venueName) setVenueName(p.venueName);
    if (p?.guestCount) setGuestCount(p.guestCount);
    if (p?.dietaryNotes) setDietaryNotes(p.dietaryNotes);
    if (p?.phone) setPhone(p.phone);
    if (p?.address) setAddress(p.address);
    if (p?.notes) setStyleNotes(p.notes);
    if (p?.stylePreferences) {
      const prefs = parsePrefs(p.stylePreferences);
      if (prefs.styles) setSelectedStyles(prefs.styles);
      if (prefs.silhouettes) setSelectedSilhouettes(prefs.silhouettes);
      if (prefs.fabrics) setSelectedFabrics(prefs.fabrics);
      if (prefs.necklines) setSelectedNecklines(prefs.necklines);
    }
  }, [me]);

  const mutation = useMutation({
    mutationFn: (payload: UpdateBrideProfilePayload) =>
  ...
  function handleSave() {
    setApiError(null);
    const stylePreferences = serializePrefs({
      styles: selectedStyles,
      silhouettes: selectedSilhouettes,
      fabrics: selectedFabrics,
      necklines: selectedNecklines,
    });
    mutation.mutate({
        ...
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13, color: "#888", margin: 0 }}>
              Help Fatima get to know you and your vision
            </p>
          </div>
            ...
            {steps.map((s, i) => (
              <button
                ...
                style={{
                  flex: 1,
                  padding: "13px 16px",
                  background: step === s.num ? "#F0E4D8" : "transparent",
                  border: "none",
            ...
            >
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <input
                  ...
                  placeholder="e.g. Pasadena Estate, Blackheath"
                  style={inputStyle}
                  onFocus={(e) =>
                    (e.currentTarget.style.borderColor = "#D4A373")
                  }
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Any notes for in-studio fittings…"
                  rows={3}
          ...
          {step === 2 && (
            ...
            >
              ...
              <StyleSection
                ...
                onToggle={(item) =>
                  toggle(selectedStyles, setSelectedStyles, item)
                }
              />
              <StyleSection
                label="Silhouette"
                subtitle="Select your preferred shape"
                items={silhouetteOptions}
                selected={selectedSilhouettes}
                onToggle={(item) =>
                  toggle(selectedSilhouettes, setSelectedSilhouettes, item)
                }
              />
              <StyleSection
                label="Fabrics"
                subtitle="Textures you're drawn to"
                items={fabricOptions}
                selected={selectedFabrics}
                onToggle={(item) =>
                  toggle(selectedFabrics, setSelectedFabrics, item)
                }
              />
              <StyleSection
                label="Neckline"
                subtitle="Select one or more"
                items={necklineOptions}
                selected={selectedNecklines}
                onToggle={(item) =>
                  toggle(selectedNecklines, setSelectedNecklines, item)
                }
              />

              <div style={{ marginTop: 20 }}>
                <label style={labelStyle}>
                  Any other notes or inspirations?
                </label>
                <textarea
                  value={styleNotes}
                  onChange={(e) => setStyleNotes(e.target.value)}
                  placeholder="Describe your dream gown, share Pinterest links, note any must-haves or deal-breakers…"
                  rows={4}
                  style={{ ...inputStyle, resize: "vertical" }}
                  onFocus={(e) =>
                    (e.currentTarget.style.borderColor = "#D4A373")
            ...
            >
              ...
              >
                ...
                <div style={{ fontSize: 13, color: "#555", lineHeight: 2 }}>
                  <div>
                    Wedding date: <strong>{weddingDate || "Not set"}</strong>
                  </div>
                  {partnerName && (
                    <div>
                  ...
                  <div>
                    Style:{" "}
                    <strong>
                      {selectedStyles.slice(0, 2).join(", ") || "Not set"}
          ...
          return (
            <button
              ...
              style={{
                padding: "6px 14px",
                borderRadius: 20,
                border: `1px solid ${active ? "#D4A373" : "#E8E0D5"}`,
                background: active ? "#F0E4D8" : "transparent",
                color: active ? "#7A5C3A" : "#666",
                fontSize: 12,
